...
seed=(10)
good <- Netflix[which (Netflix$rating!="TV-MA" & Netflix$rating!="TV-PG"),]
bad1 <- sample_n(Netflix[which (Netflix$rating=="TV-MA"),],100)
bad2 <- sample_n(Netflix[which (Netflix$rating=="TV-PG"),],100)
Netflix_Sample <- rbind (good, bad1, bad2)
p <- ggplot(data=Netflix_Sample, mapping = aes(x = For_Kids,fill = rating))
p + geom_bar() +
  labs(x = "Appropriateness", 
       y = "Movies",
       title = "Sample Breakdown of Netflix Movies Appropriateness for 13-Year-Old")
